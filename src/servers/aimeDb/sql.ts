...
import type { AimeId } from '../../utils/model'
import { generateExtId } from '../../utils/model'
import type { Transaction } from '../../utils/sql'
import type { CardRepository, Repositories } from './repo'

class CardRepositoryImpl implements CardRepository {
  constructor(private readonly _txn: Transaction) { }

  async lookup(luid: string, now: Date): Promise<AimeId | undefined> {
    const fetchSql = sql.select('p.id', 'p.ext_id').from('aime_player p').where('p.luid', luid)
    ...
    const touchSql = sql.update('aime_player').set({ access_time: now }).where('id', id)
